...
- **In-place agent upgrades** - `runners upgrade` swaps the actions/runner agent
  binaries to a newer release without re-registering (the agent's own auto-update
  mechanism), one runner at a time, each self-tested back to active or rolled back
  to its prior cached version. Busy runners and mid-cycle ephemeral lanes are
  skipped. Versions are tracked in a host manifest (`/var/lib/srm/state.json`) and
  surfaced by `runners list` and `doctor`.
